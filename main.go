...
func main() {
	...
	app.OnServe().Bind(&hook.Handler[*core.ServeEvent]{
		...
		e.Router.GET("/api/leads/stats", func(c *core.RequestEvent) error {
				...
				e.App.DB().NewQuery(todayQuery).All(&todayResults)
	...
	app.OnServe().Bind(&hook.Handler[*core.ServeEvent]{
		Func: func(e *core.ServeEvent) error {
			e.Router.GET("/api/leads/breakdown", func(c *core.RequestEvent) error {
				info, _ := c.RequestInfo()
				if info.Auth == nil {
					return c.JSON(http.StatusUnauthorized, map[string]string{
						"error": "Unauthorized",
				...
				for _, user := range users {
					...
					e.App.DB().NewQuery(query).Bind(dbx.Params{
	...
	app.OnServe().Bind(&hook.Handler[*core.ServeEvent]{
		Func: func(e *core.ServeEvent) error {
			e.Router.GET("/api/employee/stats", func(c *core.RequestEvent) error {
				info, _ := c.RequestInfo()
				if info.Auth == nil {
					return c.JSON(http.StatusUnauthorized, map[string]string{
						"error": "Unauthorized",
				...
				for _, user := range users {
					...
					e.App.DB().NewQuery(ipaQuery).Bind(dbx.Params{"code": user.EmployeeCode}).One(&ipaResult)
					e.App.DB().NewQuery(ipdQuery).Bind(dbx.Params{"code": user.EmployeeCode}).One(&ipdResult)
